...
|---|---|---|---|---|
| A-01 | **Certificacion cULus / cETL / CSA por SKU de todo dispositivo con `instalable_en_caja: true`.** Son 24 entradas del catalogo. Ninguna se puede especificar en un archivo de cliente mientras `certificacion` sea `null`: `validar.py` lo rechaza. | `catalogo/dispositivos.yaml`, campo `certificacion` | Marca impresa en la unidad fisica en recepcion; ficha tecnica del SKU norteamericano | Toda compra e instalacion |
