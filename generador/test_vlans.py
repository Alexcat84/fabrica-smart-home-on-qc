...
def renderizar_vlans(id_paquete: str) -> dict:
    ...
    assert paquete is not None, f"El paquete {id_paquete} no existe en paquetes/"
    ...
    salida = env.get_template("red/vlans.yaml.j2").render(**ctx)
...
class ManagementSeDespliegaDeLEnAdelante(unittest.TestCase):
    ...
    def test_plegar_management_no_elimina_la_regla_de_cortafuegos(self):
        ...
        ctx["generado_en"] = "prueba"
        datos = yaml.safe_load(env.get_template("red/firewall.yaml.j2").render(**ctx))
