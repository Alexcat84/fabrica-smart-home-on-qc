...
Este modulo lo lee del registro del dispositivo en `catalogo/dispositivos.yaml`
...
def desde_cliente(cliente: dict, catalogo_dispositivos: list[dict] | None = None) -> ResultadoAnchoBanda:
    ...
    `catalogo_dispositivos` es la lista de `catalogo/dispositivos.yaml`. Si no se pasa, se carga.
    """
    if catalogo_dispositivos is None:
        ...
        catalogo_dispositivos = yaml.safe_load(
            (raiz / "catalogo" / "dispositivos.yaml").read_text(encoding="utf-8")
        )
