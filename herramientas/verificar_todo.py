...
def yaml_carga() -> tuple[bool, str]:
    ...
    malos = []
    revisados = 0
    for patron in ("catalogo/*.yaml", "paquetes/*.yaml", "clientes/**/*.yaml",
                   "ansible/**/*.yml", "salida/**/*.yaml",
                   "plantillas/homeassistant/textos/*.yaml"):
...
def pruebas_validador() -> tuple[bool, str]:
    codigo, salida = ejecutar(
        [sys.executable, "-m", "unittest", "discover", "-s", "generador", "-p", "test_*.py"]
    )
    n = re.search(r"Ran (\d+) tests", salida)
...
def catalogo_coherente() -> tuple[bool, str]:
    codigo, salida = ejecutar([sys.executable, "generador/validar.py", "--catalogo"])
    return codigo == 0, salida.strip().splitlines()[-1] if salida.strip() else "sin salida"


def demo_valida() -> tuple[bool, str]:
    codigo, salida = ejecutar(
        [sys.executable, "generador/validar.py", "clientes/EJEMPLO-demo/cliente.yaml"]
    )
...
def demo_genera() -> tuple[bool, str]:
    codigo, salida = ejecutar(
        [sys.executable, "generador/generar.py", "clientes/EJEMPLO-demo/cliente.yaml"]
    )
    if codigo != 0:
...
def sin_secretos() -> tuple[bool, str]:
    codigo, salida = ejecutar([sys.executable, "herramientas/detectar_secretos.py", "--todo"])
    return codigo == 0, salida.strip().splitlines()[-1] if salida.strip() else "sin salida"
...
def regla_del_fuego() -> tuple[bool, str]:
    ...
    sospechosas = []
    for patron in ("salida/**/*", "plantillas/**/*", "paquetes/*.yaml"):
        for ruta in RAIZ.glob(patron):
...
def trazabilidad() -> tuple[bool, str]:
    ...
    disp = yaml.safe_load((RAIZ / "catalogo" / "dispositivos.yaml").read_text(encoding="utf-8"))
    ...
    if "catalogo/dispositivos.yaml" not in cola:
